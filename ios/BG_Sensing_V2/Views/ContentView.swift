...
struct ContentView: View {
    ...
    private var recordButton: some View {
        Button {
            if recordingSessionManager.isRecordingPublished {
                recordingSessionManager.stopRecording()
            } else if !recordingSessionManager.isFinalizing {
        ...
        // Disabled only in the "stopped, but the previous session is still
        // draining its writes to disk" state — tapping STOP is always live.
        .disabled(recordingSessionManager.isFinalizing && !recordingSessionManager.isRecordingPublished)
        .padding(.horizontal, 24)
    }

    private var recordButtonLabel: String {
        if recordingSessionManager.isRecordingPublished { return "STOP RECORDING" }
        if recordingSessionManager.isFinalizing { return "FINISHING RECORDING…" }
        return "START RECORDING"
    }

    private var recordButtonColor: Color {
        if recordingSessionManager.isRecordingPublished { return .red }
        if recordingSessionManager.isFinalizing { return .gray }
        return .green
    }
